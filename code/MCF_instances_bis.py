...
class MCF_graph:
    ...
    def setup(self, path_demande, path_noeud, path_edge):
        ...
        for row in rows:
            row1 = row[0].split(';')
            row2 = []
            row2.append(int(row1[0]))
            row2.append(int(row1[1])-1)
            row2.append(int(row1[2])-1)
            row2.append(int(row1[4]))
            row2.append(float(row1[3]))
        ...
        for row in rows2:
            rowA = row[0].split(';')
            rowB = []
            rowB.append(float(rowA[1])-1)
            rowB.append(float(rowA[2])-1)
            rowB.append(float(rowA[5]))
        ...
        for row in rows1:
            ...
            row3 = row[0].split(';')
            row4 = []
            row4.append(int(row3[0]))
            row4.append(1)
            row4.append(row3[1])
            row4.append(0)
            row4.append(float(row3[2]))
            row4.append(float(row3[3]))
            row4.append(0)
    ...
    def find_ze(self):
        ...
        D = [self.DEMANDE[i][2] for i in range(self.nombre_demande)]
        ...
        Y = []
        for i in range(self.nombre_demande):
            Y.append(LpVariable('y' + str(i), lowBound=0))

        Lp_prob = LpProblem('Problem', LpMinimize)
        Lp_prob += sum([Z[i] * Capacite[i] for i in range(self.nombre_edge)])

        for demand in range(self.nombre_demande):
            paths = self.create_path(demand)
            for path in paths:
                expr = 0
                ...
                expr = expr >= Y[demand]
                Lp_prob += expr
        Lp_prob += sum([D[i] * Y[i] for i in range(self.nombre_demande)]) >= 1
        status = Lp_prob.solve()
        VALUEZ = [value(Z[i]) for i in range(self.nombre_edge)]
        print('VALUEZ ::: ', VALUEZ)
        return VALUEZ, value(Lp_prob.objective)

    def create_matrix(self):
        VALUEZ, valueopt = self.find_ze()
        weight = VALUEZ
...
g = MCF_graph('Nouvelles-instances/demand_cost266_capa_100_uniform_medium.csv', 'Nouvelles-instances/nodes_cost266_capa_100_uniform_medium.csv', 'Nouvelles-instances/edges_cost266_capa_100_uniform_medium.csv')
...
test_sp = []
#normal_sp = g.find_sp_for_all_coordinates(False)
#print('normal :: ', normal_sp)
random_sp = g.find_sp(False)
