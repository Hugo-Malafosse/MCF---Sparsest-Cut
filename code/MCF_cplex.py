...
def dijkstra(origin):
    ...
    labels = np.full((nStations),
                     np.inf)  # Contains the cost of the shortest paths from origin with init value = infinity
    labels[origin] = 0
    toTreat = np.array([origin])  # Node that need to be processed

    while True:
        currentNode = toTreat[
            np.argmin(np.take(labels, toTreat))]  # Pick the node with lowest shortest path from the set toTreat
        for i in range(nStations):
            if (cost[currentNode, i, 0] > 0):  # Among all neighbors of currentNodes
                if (labels[i] > labels[currentNode] + cost[currentNode, i, 0]):  # If dual constraint is violated
                    ...
                    labels[i] = labels[currentNode] + cost[
                        currentNode, i, 0]  # Lower it to satisfy the feasibility and complementary slackness
                    toTreat = np.append(toTreat,
                                        i)  # As we found a better path to go to i, need to check on the neighbors of i
...
def printAllPath(i, precedent, j, visited, arc, costP, optimal):
    ...
    if (precedent > -1):
        arc[cost[
            precedent, i, 1]] = 1  # If you moved from a previous node, add the arc between them to remember the pathway
    ...
    if (
            precedent > -1):  # Once you reached a leaf (no where to go), reset arc and visited variable to their previous value
...
def dual_of_Restrited(pathsK):
    ...
    for i in range(nCommodities):
        model.variables.add(obj=[1], lb=[-cplex.infinity], ub=[cplex.infinity],
    ...
    for i in range(nCommodities):
        for j in range(len(pathsK[i])):
            ...
            model.linear_constraints.add(lin_expr=row,
                                         ...
                                         rhs=[pathsK[i][j][nArcs] * float(
                                             commodities[i][2])])  # Right Hand Side of the constraint
    ...
    return model.solution.get_values()[:nCommodities], model.solution.get_values()[
                                                       nCommodities:nCommodities + nArcs], model.solution.get_values()[
                                                                                           nArcs + nCommodities:]
...
def restricted_Master(pathsK):
    ...
    for i in range(nCommodities):
        for j in range(len(pathsK[i])):
            model.variables.add(obj=[pathsK[i][j][nArcs] * float(commodities[i][2])], lb=[0], ub=[1],
                                # obj contains the coefficients of the decision variables in the objective function
                                types=[model.variables.type.continuous],
                                names=['P(' + str(i) + ',' + str(j) + ')'])
    ...
    for i in range(nArcs):
        ...
        model.linear_constraints.add(lin_expr=row,
                                     senses="L",  # Less-than
                                     rhs=[capacity_arc[
                                              arc[i][3] - 1]])  # Capacity of the arc (-1 because type_id start at 1)
    ...
    for i in range(nStations):
        ...
        for j in range(nCommodities):  # For each commodity path, which nodes it contains (do not include starting node)
            for k in range(len(pathsK[j])):
                for l in range(nArcs):
                    if (pathsK[j][k][l] == 1 and arc[l][
                        1] - 1 == i):  # If it is the case, add the decision variable index to the constraint
                        ind.append(count)
                        val.append(commodities[j][2])  # With its coefficiant
                count += 1
        row.append([ind, val])
        model.linear_constraints.add(lin_expr=row,
                                     senses="L",  # Less-than
                                     rhs=[capacity_node[
                                              node[i][2] - 1]])  # Capacity of the node (-1 because type_id start at 1)
    ...
    try:
        ...
        for i in range(nCommodities):
            for j in range(len(pathsK[i])):
                indices = [k for k, x in enumerate(pathsK[i][j]) if
                           x == 1]  # Get the indices of the arcs contained in the current path
                print("\t", model.solution.get_values(count) * commodities[i][2], "quantity of commodity n°", i,
                      "on path", node[commodities[i][0]][0], ''
                      + ' '.join([node[arc[k][1] - 1][0] for k in indices]) + ". Length path : " + str(
                        pathsK[i][j][nArcs]))
                count += 1

        print("\nTotal cost = " + str(model.solution.get_objective_value()))

        dualCommodities, dualArcs, dualStations = dual_of_Restrited(
            pathsK)  # Compute the dual variables and print the non-zero ones
    ...
    for i in range(nCommodities):  # For each commodity, solve the shortest path problem (with updated cost vector)
        ...
        for k in range(nStations):
            ...
            if (commodities[i][0] != k and commodities[i][1] != k):  # If the node is in between
                ...
                for j in range(nArcs):
                    if (arc[j][
                        0] - 1 == k):  # Compute its leaving flow (-1 because in the data we start with index 1 and not 0)
                        ind.append(j)
                        val.append(1)
        ...
        try:
            ...
            if (
                    round(model.solution.get_objective_value()) < reducedCost):  # If we obtained a more violated constraint, take it
...
for i in range(
        nStations - 1):  # Don't need the last node, if you have already find all the shortest path from every node except the last, you already computed the info for the last node
...
while True:
    ...
    while True:
        obj_Function, solution, dualCommodities, dualArcs, dualStations = restricted_Master(
            pathsK)  # Solve the restricted master problem
    ...
    reducedCost, newPath, forCommodity = pricingProblem(dualCommodities, dualArcs,
                                                        dualStations)  # Solve the pricing problem
    ...
    if (round(reducedCost) == 0):  # round to avoid minor computational error (10^-23 instead of 0)
        ...
        for i in range(nCommodities):
            ...
            for j in range(len(pathsK[i])):
                indices = [k for k, x in enumerate(pathsK[i][j]) if
                           x == 1]  # Get the indices of the arcs contained in the current path
                print("\t", solution[count] * commodities[i][2], "quantity of commodity n°", i + 1, "on path",
