...
class MCF_graph_raf:
    ...
    def find_ze(self):
        ...
        for demand in range(self.nombre_demande):
            #print(demand / self.nombre_demande)
    ...
    def partition_raf(self, random1):
        logn = int(np.log(self.nombre_noeuds))
        ENS = []
        source_puits = []
        for i in range(len(self.DEMANDE)):
            dem = self.DEMANDE[i]
            if int(dem[0]) not in source_puits:
                source_puits.append(int(dem[0]))
            if int(dem[1]) not in source_puits:
                source_puits.append(int(dem[1]))
    ...
    def compute_plongement(self, random1):
        ENS = self.partition_raf(random1)
        #print('ENSSSS ///// ', ENS)
        vects = []
        M = self.create_matrix()
        for j in range(self.nombre_noeuds):
...
g = MCF_graph_raf('../graphe normal/demande.csv', 'noeux.csv', 'edge.csv')
#g = MCF_graph_raf('Graphe biparti/graphs/Demands_bipartite(2, 3).csv', 'Graphe biparti/graphs/Nodes_bipartite(2, 3).csv', 'Graphe biparti/graphs/Edges_bipartite(2, 3).csv')
